...
object TestHarness {
  val system = ActorSystem("GroupServer")
  implicit val timeout = Timeout(60 seconds)
  val numNodes = 10//total number of servers in the app tier
  val numGroups = 5
  val burstSize =5 //1000//number of commands per burst
  val opsPerNode = 100

  // Service tier: create app servers and a Seq of per-node Stats
  val master = KVAppService(system, numNodes, burstSize, numGroups)
